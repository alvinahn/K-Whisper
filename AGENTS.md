...
Project-specific instructions for Codex working on this repo.
...
- **STT provider**: Groq Whisper Large-v3-Turbo (distilled, speed-optimized). Korean fidelity (dialect preservation, confirmation `?`) is handled downstream by the Verbatim default mode + `PunctuationRestorer`, not at the STT layer. Pipeline auto-falls-back to whichever provider key is present.
- **Default mode**: **Verbatim** (no LLM step — Superwhisper-style raw STT passthrough). Glossary substitution still runs; LLM is skipped. Migration v3 bumps existing `default-cleanup` / `raw` users to `verbatim`.
- **Cleanup mode** (opt-in, formerly "Default cleanup"): Groq `llama-3.3-70b-versatile` (NOT 8B — 70B handles Korean morphology far better). Conservative prompt + `PunctuationRestorer` post-pass. User picks this when they actually want STT errors auto-fixed.
...
- Always pass Korean-or-mixed audio through `whisper-large-v3-turbo` (Groq) or `whisper-large-v3` (full). The older OpenAI `whisper-1` API is **not acceptable** for Korean — it's the v2-era model with weak Korean.
- Default is Verbatim — **the LLM cleanup is the single biggest source of unwanted normalization** (사투리 → 표준어, ~데 → ~네, 같애 → 같아, `?` stripping). Skipping it is the architectural fix; superwhisper.com's "Voice to Text" default works the same way.
- Cleanup mode (opt-in) prompt catches verb-merge errors (e.g. `미치고 버렸네` → `미쳐버렸네`), particle confusion. Heavy "preserve dialect / register / endings / punctuation" rules; deterministic `PunctuationRestorer` re-inserts `?`/`.`/`!` Llama 70B sometimes strips.
- Glossary terms inject into BOTH the STT `prompt` field AND (in Cleanup mode) the LLM system prompt. Glossary alias substitution (`name|alias1,alias2`) is deterministic and runs in BOTH modes pre-LLM.
