...
enum DefaultModes {
    static let all: [Mode] = [
        Mode(
            ...
            Korean correction rules:
            - Verb-ending merge errors. When "[verb]+고 [aux verb]" looks ungrammatical, prefer the contracted form "[verb-stem]+어/아[aux verb]". For instance, an STT output that reads "미치고 버렸네" is most likely supposed to be "미쳐버렸네".
            - Particles: pick the right one from context (이/가, 은/는, 을/를, 에/에서, 으로/로).
            - Compound endings stay whole (했어요, 했었어요, 했네요, 했는데).

            English correction: remove fillers (um, uh, like, you know), fix punctuation and capitalization.

            Preserve the speaker's meaning. Do not paraphrase. Do not translate.

            STRICT OUTPUT FORMAT:
            Reply with ONLY the corrected transcript. No commentary. No quotation marks. No arrows. No "before / after" or "X → Y" listing. Do not include the original input alongside the result. Just the single cleaned sentence(s).
            """,
            provider: .groq,
