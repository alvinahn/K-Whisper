...
///
/// Activation is **delayed** by `activationDelayMs`: the key must be held alone
/// for that long before "pressed" fires. This prevents a quick chord like
/// right-Cmd + Backspace from accidentally triggering recording.
final class HoldKeyMonitor {
    typealias Handler = (Bool) -> Void  // true = activated, false = released after activation

    private var globalMonitor: Any?
    private var localMonitor: Any?
    private var isPhysicallyDown = false  // tracks the raw key state
    private var isActive = false           // tracks whether we've fired "pressed"
    ...
    private func evaluate(event: NSEvent) {
        ...
        if down {
            // Schedule activation; if user releases (or chords with another key) before delay
            // elapses, we'll cancel below in the `else` branch.
            ...
            // If we never activated (quick tap), do nothing — release is silent.
        }
    }
}
